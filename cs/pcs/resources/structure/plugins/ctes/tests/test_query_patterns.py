...
from cs.pcs.resources.structure.plugins import ctes


@pytest.mark.unit
class Utility(unittest.TestCase):
    @mock.patch.object(ctes, "open", create=True)
    @mock.patch.object(ctes.os.path, "abspath")
    @mock.patch.object(ctes.os.path, "dirname")
    @mock.patch.object(ctes.misc, "jail_filename")
    def test_load_query_pattern(self, jail_filename, dirname, abspath, mock_open):
        ...
        self.assertEqual(
            ctes.load_query_pattern("foo"),
            read.return_value,
        )
        mock_open.assert_called_once_with(jail_filename.return_value, "r", encoding="utf-8")
        read.assert_called_once_with()
        jail_filename.assert_called_once_with(abspath.return_value, "foo")
        abspath.assert_called_once_with(dirname.return_value)
        dirname.assert_called_once_with(ctes.__file__)

    @mock.patch("cdb.mssql.CollationDefault.get_default_collation")
    @mock.patch.object(ctes, "load_query_pattern")
    @mock.patch.object(ctes.sqlapi, "SQLdbms", return_value="MSSQL")
    @mock.patch.object(ctes.sqlapi, "DBMS_MSSQL", "MSSQL")
    def test_get_query_pattern_mssql(
        self, SQLdbms, load_query_pattern, get_default_collation
    ):
        self.assertEqual(
            ctes.get_query_pattern("pattern"),
    ...
    @mock.patch.object(ctes, "load_query_pattern")
    @mock.patch.object(ctes.sqlapi, "SQLdbms", return_value="LITE")
    @mock.patch.object(ctes.sqlapi, "DBMS_SQLITE", "LITE")
    ...
    @mock.patch.object(ctes, "load_query_pattern")
    @mock.patch.object(ctes.sqlapi, "SQLdbms", return_value="ORA")
    @mock.patch.object(ctes.sqlapi, "DBMS_ORACLE", "ORA")
    ...
    @mock.patch.object(ctes, "load_query_pattern")
    @mock.patch.object(ctes.sqlapi, "SQLdbms", return_value="?")
