...
from cs.pcs.taskboards.team_interval_board.issue_card_adapter import (
    TeamIntervalBoardIssueAdapter,
)
...
@pytest.mark.unit
class TestTeamIntervalBoardIssueAdapter(unittest.TestCase):
    @mock.patch.object(TeamIntervalBoardIssueAdapter, "set_due_date")
    def test_on_change_position_post_COLUMN_READY(self, set_due_date):
        "Team Interval Board: Change position from Issue card to column 'READY'"
        # create board
        ...
        board_adapter = mock.MagicMock(autospec=TeamIntervalBoardIssueAdapter)
        ...
        TeamIntervalBoardIssueAdapter.on_change_position_post(board_adapter, card)
    ...
    def test_on_change_position_post_COLUMN_DOING(self, set_due_date):
        "Team Interval Board: Change position from Issue card to column 'DOING'"
        # create board
        board = mock.MagicMock(autospec=Board)
        board.get_present_timeframe.return_value = ("start", "end")
