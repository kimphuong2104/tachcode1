...
class MigrateCategories(object):
    ...
    kGeneratedCategoryDesc = (
        "Automatically generated by the update task "
        "cs.documents.updates.v15_2_0_2.MigrateCategories"
    )
    kCategoryAttrPrefix = "z_categ"
    kCategoryExp = re.compile(
        r"^(?:zeichnung\.)?%s(?P<level>\d+)$" % kCategoryAttrPrefix
    )
    ...
    def find_categories_by_name(self, *names):
        ...
        def find_category_by_name(name_d, parent_id):
            ...
            cat = documents.DocumentCategory.ByKeys(
                categ_id=name_d, parent_id=parent_id
            )
            if cat:
                return [cat.categ_id]
        ...
        for name in names:
            ...
            if len(cats) >= 1:
                if len(cats) > 1:
                    protocol.logWarning(
                        "Found more than one document category for %s. "
                ...
                protocol.logWarning(
                    "Cannot find a document category for %s. "
                    "A new one will be created" % "/".join(names[: len(result) + 1])
                )
                missing_categories = names[len(result) :]
                return result + self.create_categories(
                    *missing_categories, parent_id=parent
                )
    ...
    def migrate_favorite(self, fav):
        ...
        if fav and fav.fav_link:
            # E052039: There seems to be favorites that contain a #
            #          We replace the # to make urlparse work correct because
            #          there is no regular use case for #
            url = urlparse(fav.fav_link.replace("#", "%23"))
            exp = (
            ...
            if re.match(exp, url.path):
                query = dict(parse_qsl(url.query.encode("utf-8")))
    ...
    def run(self):  # pylint: disable=no-self-use
        ...
        with transactions.Transaction():
            ...
            for record in records_set:
                names = [_f for _f in [getattr(record, attr) for attr in attrs] if _f]
                ...
                if names and cats and names != cats:
                    ...
                    if len(names) != len(columns):
                        cond = " AND ".join(
                            [
                                "(z_categ{lvl} IS NULL OR z_categ{lvl} = '')".format(
                                    lvl=lvl + 1
                                )
                                for lvl in range(len(names), len(columns))
                            ]
                        )
                        docs = documents.Document.Query(cond)
                    else:
                        docs = documents.Document.Query()
                    docs = docs.KeywordQuery(**dict(list(zip(attrs, names))))
                    docs.Update(**dict(list(zip(attrs, cats))))

            # migrate favorites
            try:
                from cdb.platform import favourite
            except ImportError:
                # There are no favorites with CE 16
                return
