...
class SelfDeprecatingDict(object):
    ...
    def __getattr__(self, attribute):
        if attribute in self.dict_only:
            warnings.warn(
                "In future this will be a list and not a dictionary", DeprecationWarning
            )
    ...
    def __init__(self, ole_object, method_name):
        ...
        try:
            ...
            if self.in_parameters is None:
                ...
                self.in_parameter_names = [
                    (i.Name, i.IsArray) for i in self.in_parameters.Properties_
                ]
            if self.out_parameters is None:
                self.out_parameter_names = []
            else:
                self.out_parameter_names = [
                    (i.Name, i.IsArray) for i in self.out_parameters.Properties_
                ]
    ...
    def __call__(self, *args, **kwargs):
        ...
        try:
            if self.in_parameters:
                ...
                #
                for n_arg in six.moves.range(len(args)):
                    arg = args[n_arg]
                ...
                result = self.ole_object.ExecMethod_(
                    self.method.Name, self.in_parameters
                )
            else:
...
class _wmi_object:
    ...
    def __init__(self, ole_object, instance_of=None, fields=[], property_map={}):
        try:
            ...
            _set(self, "_properties", list(self.properties))
            _set(self, "_methods", list(self.methods))
            _set(
                self,
                "qualifiers",
    ...
    def __repr__(self):
        ...
        try:
            return "<%s: %s>" % (
                ...
                self.Path_.Path.encode("ascii", "backslashreplace"),
            )
        except pywintypes.com_error:
            handle_com_error()
    ...
    def __getattr__(self, attribute):
        ...
        try:
            if attribute in self.properties:
                ...
                factory = self.property_map.get(
                    attribute, self.property_map.get(property.type, lambda x: x)
                )
    ...
    def _getAttributeNames(self):
        ...
        attribs = [str(x) for x in list(self.methods)]
        attribs.extend([str(x) for x in list(self.properties)])
        return attribs
        ...
        if self._associated_classes is None:
            ...
            try:
                associated_classes = dict(
                    (assoc.Path_.Class, _wmi_class(self._namespace, assoc))
                    for assoc in self.ole_object.Associators_(**params)
                )
    ...
    def associators(self, wmi_association_class="", wmi_result_class=""):
        ...
        try:
            return [
                _wmi_object(i)
                for i in self.ole_object.Associators_(
                    strAssocClass=wmi_association_class, strResultClass=wmi_result_class
                )
            ]
        except pywintypes.com_error:
            handle_com_error()
    ...
    def references(self, wmi_class=""):
        ...
        try:
            return [
                _wmi_object(i)
                for i in self.ole_object.References_(strResultClass=wmi_class)
            ]
        except pywintypes.com_error:
            handle_com_error()
...
class _wmi_event(_wmi_object):
    ...
    def __init__(self, event, event_info, fields=[]):
        ...
        if event_info:
            event_type = (
                self.event_type_re.match(event_info.Path_.Class).group(1).lower()
            )
...
class _wmi_class(_wmi_object):
    ...
    def __init__(self, namespace, wmi_class):
        ...
        if namespace:
            ...
            namespace = _wmi_namespace(
                GetObject(winmgmts + ":" + namespace_moniker), False
            )
            _set(self, "_namespace", namespace)
    ...
    def watch_for(
        self, notification_type="operation", delay_secs=1, fields=[], **where_clause
    ):
        if self._namespace is None:
        ...
        if notification_type.lower() not in valid_notification_types:
            raise x_wmi(
                "notification_type must be one of %s"
                % ", ".join(valid_notification_types)
            )
...
class _wmi_namespace:
    ...
    def _cached_classes(self, class_name):
        ...
        if class_name not in self._classes_map:
            self._classes_map[class_name] = _wmi_class(
                self, self._namespace.Get(class_name)
            )
...
def connect_server(
    ...
    if impersonation_level:
        try:
            impersonation = getattr(
                obj._constants, "wbemImpersonationLevel%s" % impersonation_level.title()
            )
        except AttributeError:
            raise x_wmi_authentication(
    ...
    server = Dispatch("WbemScripting.SWbemLocator").ConnectServer(
        server,
        namespace,
        user,
        password,
