...
class ProjectWSJobExec(JobExecBase, Singleton):
    CAD_SYSTEM = "MS-Project"
    PROJECT_EXE = "WINPROJ.EXE"
    PROJECT_ADDIN_NAME = "CONTACT MS Office Link (MS-Project)"
    ...
    def get_office_addin(self, uninitializeCOM=True):
        ...
        pythoncom.CoInitialize()
        try:
            # Disable the Office start screen for MS Project.
            # Otherwise, 'Start Screen' blocks MS Project and MS Project waits for input from user
            # Set REG_KEY: SOFTWARE\Microsoft\Office\16.0\MS Project\Options\General\DisableBootToOfficeStart -> 1
            key = six.moves.winreg.OpenKey(six.moves.winreg.HKEY_CURRENT_USER,
                                           r"SOFTWARE\Microsoft\Office\16.0\MS Project\Options\General",
            ...
            print(r"Couldn't set REG_KEY: \
                   SOFTWARE\Microsoft\Office\16.0\Word\Options\General\DisableBootToOfficeStart -> 1")

        try:
            wordClient = win32com.client.GetObject(None, "MSProject.Application")

            if wordClient is not None:
                try:
                    for wordAddin in wordClient.COMAddIns:
                        description = wordAddin.Description
                        if(description == self.PROJECT_ADDIN_NAME):
                            try:
                                addInObject = wordAddin.Object
                                break
                            except BaseException:
                                print("Exception: Couldn't get the COM object for '%s'"
                                      % self.PROJECT_ADDIN_NAME)
                except BaseException:
        ...
        if addInObject is not None:
            print("The COM Object for '%s' was found successfully"
                  % self.PROJECT_ADDIN_NAME)
        return addInObject
    ...
    def get_cad_binary_path(self, version):
        ...
        key = six.moves.winreg.OpenKey(
            ...
            r"SOFTWARE\Microsoft\Office\%s\Project\InstallRoot"
            % version)
        value = six.moves.winreg.QueryValueEx(key, "Path")
        cad_binary_path = r"%s\%s" % (value[0], self.PROJECT_EXE)
