...
class PowerPointWSJobExec(JobExecBase, Singleton):
    CAD_SYSTEM = "MS-PowerPoint"
    POWERPOINT_EXE = "POWERPNT.EXE"
    POWERPOINT_ADDIN_NAME = "CONTACT MS Office Link (MS-PowerPoint)"
    ...
    def get_office_addin(self, uninitializeCOM=True):
        ...
        """
        powerPointAddin = None
        powerPointClient = None
        addInObject = None

        pythoncom.CoInitialize()
        try:
            powerPointClient = win32com.client.GetObject(None, "PowerPoint.Application")

            if powerPointClient is not None:
                try:
                    for powerPointAddin in powerPointClient.COMAddIns:
                        description = powerPointAddin.Description
                        if(description == self.POWERPOINT_ADDIN_NAME):
                            try:
                                addInObject = powerPointAddin.Object
                                break
                            except BaseException:
                                print("Exception: Couldn't get the COM object for '%s'"
                                      % self.POWERPOINT_ADDIN_NAME)
                except BaseException:
                    print("Exception: unable to determine the COM object for '%s'"
                          % self.POWERPOINT_ADDIN_NAME)
                    addInObject = None
        except (pywintypes.com_error):
            print("Couldn't get com object for PowerPoint.Application")

        if uninitializeCOM:
            pythoncom.CoUninitialize()
        if addInObject is not None:
            print("The COM Object for '%s' was found successfully" % self.POWERPOINT_ADDIN_NAME)
        return addInObject
    ...
    def call(self, app_job):
        ...
        if not self.is_cad_running():
            raise WsmException(u"POWERPOINT is not running, cannot execute command!")
    ...
    def shutdown(self, job_work_dir):
        """
        Send a SHUTDOWN command to PowerPoint
    ...
    def get_cad_binary_path(self, version):
        ...
        key = six.moves.winreg.OpenKey(six.moves.winreg.HKEY_LOCAL_MACHINE,
                                       r"SOFTWARE\Microsoft\Office\%s\PowerPoint\InstallRoot"
                                       % version)
        value = six.moves.winreg.QueryValueEx(key, "Path")
        cad_binary_path = r"%s\%s" % (value[0], self.POWERPOINT_EXE)
