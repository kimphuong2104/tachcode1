...
class TemplateCreateApp(BaseApp):
    ...
    MOUNT_PATH = "/cs-ec-web-TemplateCreateApp"
    JS_NAMESPACE = "cs-ec-web-template_create_app"
...
class EcBaseModel(BaseModel):
    catalogs = ["cdbecm_ec_templates"]
    operation = "cdbecm_create_ec_from_template"
    classname = "cdbecm_ec"
...
@TemplateCreateApp.path(path="/ec", model=EcBaseModel)
...
def _setup(self, request):
    request.app.include("cs-ec-web-template_create_app", "0.0.1")
    return "cs-ec-web-template_create_app-TemplateCreate"
...
class GetCopyOpPara(object):
    ...
    def get_op_para(self, request):
        ...
        if oi:
            template = EngineeringChange.ByKeys(cdb_object_id=uuid)
            opData["args"] = {
                constants.kArgumentTemplateUUID: uuid,
                'template': 0,  # must be set here. if changed in pre mask the mask will not appear in webui
...
@TemplateCreateApp.path(path="op_para", model=GetCopyOpPara)
...
def _register_libraries():
    lib = static.Library("cs-ec-web-template_create_app", "0.0.1",
                         os.path.join(os.path.dirname(__file__), 'js', 'build'))
    lib.add_file("cs-ec-web-template_create_app.js")
    lib.add_file("cs-ec-web-template_create_app.js.map")
