...
class IntervalBoardActionColumnMapper(DateColumnMapper):
...
class IntervalBoardActionCardAdapter(CardAdapter):
    COLUMN_MAPPER = IntervalBoardActionColumnMapper
    ...
    def on_change_position_post(cls, board_adapter, card):
        ...
        if task and sprint:
            if board_adapter.get_column_by_type(COLUMN_DOING) == card.Column:
                if not task.end_time_plan and sprint.end_date:
                    task.end_time_plan = sprint.end_date
            elif (
                board_adapter.get_column_by_type(COLUMN_READY) == card.Column
                and task.end_time_plan
            ):
                task.end_time_plan = ""
